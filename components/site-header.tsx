...
const subCategories: Record<string, { label: string; slug: string }[]> = {
  ...
  home: [
    ...
    { label: 'Kitchen & Dining', slug: 'home-kitchen' },
    { label: 'Textiles & Bedding', slug: 'home-textiles' },
    { label: 'Storage & Organisation', slug: 'home-storage' },
    { label: 'Garden & Outdoor', slug: 'home-garden' },
    { label: 'Art & Prints', slug: 'home-art' },
  ],
}
...
function BagIcon() {
  return <svg {...iconProps}><path d="M6 2 3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z" /><line x1="3" y1="6" x2="21" y2="6" /><path d="M16 10a4 4 0 0 1-8 0" /></svg>
}
...
function PlusIcon() {
  return <svg {...iconProps} width={16} height={16}><line x1="12" y1="5" x2="12" y2="19" /><line x1="5" y1="12" x2="19" y2="12" /></svg>
}
          ...
          {LANGUAGES.map((l) => (
            <Link
              ...
              className={`w-full text-left px-3 py-1.5 text-sm hover:bg-stone-50 flex items-center ${
                l.code === currentLocale ? 'text-[#FF5722] font-medium' : 'text-stone-700'
              }`}
            >
    ...
    <header className="bg-white shadow-sm border-b sticky top-0 z-40">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
        <div className="flex flex-wrap lg:flex-nowrap items-center gap-2 lg:gap-3">
          <Link href={`/${locale}`} className="text-2xl font-black tracking-tight select-none whitespace-nowrap">
            <span className="text-stone-900">lu</span>
            <span className="text-[#FF5722]">a</span>
            <span className="text-stone-900">m</span>
          </Link>

          <form action={`/${locale}`} method="get" className="w-full sm:w-auto sm:flex-1 max-w-sm lg:max-w-md flex gap-2">
            <input
              type="text"
              name="q"
              placeholder="Search"
          ...
          <div className="w-full lg:w-auto lg:ml-auto flex flex-wrap items-center gap-1 justify-start lg:justify-end">
            {user ? (
              <>
                ...
                <IconLink href={`/${locale}/messages`} label="Messages"><MessageIcon /></IconLink>
                <IconLink href={`/${locale}/favorites`} label="Wishlist"><HeartIcon /></IconLink>
                <IconLink href={`/${locale}/orders`} label="My orders"><BagIcon /></IconLink>
                <IconLink href={`/${locale}/profile`} label="My account"><UserIcon /></IconLink>
                <LanguagePicker />
                <form action={`/${locale}/auth/signout`} method="post">
                  <button type="submit" className="text-xs text-stone-500 hover:text-stone-900 px-2 py-1.5 ml-1">
                    Sign out
                  </button>
                </form>
              </>
              ...
              <>
                <LanguagePicker />
                <Link href={`/${locale}/auth/login`} className="text-sm text-stone-700 hover:text-stone-900 px-2">Sign In</Link>
                <Link href={`/${locale}/auth/signup`} className="bg-[#FF5722] text-white text-sm px-3 py-1.5 rounded-md hover:bg-[#E64A19]">Sign Up</Link>
              </>
            )}
          </div>
        </div>
            ...
            return (
              <Link
                ...
                className={`text-sm px-3 py-1.5 rounded-full ${
                  isActive ? 'bg-[#FF5722] text-white hover:bg-[#E64A19]' : 'bg-gray-100 hover:bg-gray-200 text-gray-700'
                }`}
              >
                {c.label}
              </Link>
