...
function ChevronDown({ active }: { active: boolean }) {
  return (
    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className={`transition-transform duration-200 ${active ? 'rotate-180' : ''}`}>
  ...
  return (
    <div className="relative" ref={ref} style={{ zIndex: 9999 }}>
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="inline-flex items-center gap-1 h-9 w-9 justify-center rounded-full text-stone-700 hover:text-[#FF5722] hover:bg-stone-100 transition-colors"
      >
        <GlobeIcon />
      </button>
      {open && (
        <div className="absolute right-0 mt-1 w-36 bg-white border border-stone-200 rounded-xl shadow-xl py-1" style={{ zIndex: 9999 }}>
          {LANGUAGES.map((lang) => (
            <Link
              ...
              className={`w-full text-left px-3 py-1.5 text-sm hover:bg-stone-50 flex items-center ${lang.code === currentLocale ? 'text-[#FF5722] font-medium' : 'text-stone-700'}`}
            >
    ...
    <div className="flex items-center gap-2">
      <form action={`/${locale}`} method="GET" className="relative flex-1">
        <input
          ...
          className="w-full h-9 pl-9 pr-4 text-sm bg-stone-100 border border-stone-200 rounded-full text-stone-900 placeholder-stone-500 focus:outline-none focus:ring-2 focus:ring-[#FF5722] focus:bg-white transition-colors"
        />
        <button
          type="submit"
          className="absolute left-0 top-0 h-9 w-9 flex items-center justify-center text-stone-500 hover:text-[#FF5722] transition-colors"
      ...
      <button
        ...
        className="inline-flex items-center justify-center h-9 w-9 flex-shrink-0 bg-stone-100 border border-stone-200 rounded-full text-stone-700 hover:text-[#FF5722] hover:bg-stone-50 transition-colors"
  ...
  return (
    <nav className="border-t border-stone-100 bg-white relative" style={{ zIndex: 100 }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <ul className="flex items-center overflow-x-auto scrollbar-hide sm:overflow-visible">
            ...
            return (
              ...
              >
                <Link
                  ...
                  className={`flex items-center gap-1 px-4 py-2 text-sm font-medium whitespace-nowrap border-b-2 transition-colors duration-150 ${isActive ? 'text-[#FF5722] border-[#FF5722]' : 'text-stone-600 border-transparent hover:text-[#FF5722]'}`}
                >
            ...
            {user ? (
              <>
                ...
                <Link
                  ...
                  className="inline-flex items-center gap-1 bg-[#FF5722] text-white text-sm font-medium px-3 py-2 rounded-full hover:bg-[#E64A19] transition-colors shadow-sm mr-1"
                >
                  <PlusIcon />
                  <span className="hidden sm:inline">{tNav('sell')}</span>
                </Link>
              ...
              <>
                <LanguagePicker />
                <Link href={`/${locale}/auth/login`} className="text-sm text-stone-700 hover:text-stone-900 px-2">
                  {tNav('signin')}
                </Link>
                <Link href={`/${locale}/auth/signup`} className="bg-[#FF5722] text-white text-sm px-4 py-2.5 rounded-full hover:bg-[#E64A19] transition-colors shadow-sm">
                  {tNav('signup')}
                </Link>
              </>
            )}
        ...
        <div className="bg-stone-50 border-b border-stone-100 px-4 sm:px-6 lg:px-8 py-3">
          <div className="max-w-7xl mx-auto">
            <form action={`/${locale}`} method="get" className="flex flex-wrap items-center gap-2">
              ...
              <button type="submit" className="bg-[#FF5722] text-white px-4 py-1.5 text-sm rounded-lg font-medium hover:bg-[#E64A19] transition-colors shadow-sm">
                {tHome('apply') || 'Apply'}
              </button>
            </form>
          </div>
