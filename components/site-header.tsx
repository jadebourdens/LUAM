...
type SubCategory = { slug: string; label: LocaleString }
type Category = { slug: string; label: LocaleString; children: SubCategory[] }
type NotifRow = { id: string; title: string; message: string; link?: string; created_at: string }
...
function StoreIcon() {
  return <svg {...BASE_ICON}><rect x="3" y="3" width="7" height="7" /><rect x="14" y="3" width="7" height="7" /><rect x="14" y="14" width="7" height="7" /><rect x="3" y="14" width="7" height="7" /></svg>
}
function MessageIcon() {
  return <svg {...BASE_ICON}><path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z" /></svg>
}
function HeartIcon() {
  return <svg {...BASE_ICON}><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" /></svg>
}
function BagIcon() {
  return <svg {...BASE_ICON}><path d="M6 2 3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z" /><line x1="3" y1="6" x2="21" y2="6" /><path d="M16 10a4 4 0 0 1-8 0" /></svg>
}
function UserIcon() {
  return <svg {...BASE_ICON}><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" /></svg>
}
function GlobeIcon() {
  return <svg {...BASE_ICON}><circle cx="12" cy="12" r="10" /><line x1="2" y1="12" x2="22" y2="12" /><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z" /></svg>
}
function PlusIcon() {
  return <svg {...BASE_ICON} width={16} height={16}><line x1="12" y1="5" x2="12" y2="19" /><line x1="5" y1="12" x2="19" y2="12" /></svg>
}
function FilterIcon() {
  return <svg {...BASE_ICON} width={18} height={18}><line x1="4" y1="6" x2="20" y2="6" /><line x1="4" y1="12" x2="20" y2="12" /><line x1="4" y1="18" x2="20" y2="18" /></svg>
}
function SearchIcon() {
...
function ChevronDown({ active }: { active: boolean }) {
  return <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className={`transition-transform duration-200 ${active ? 'rotate-180' : ''}`}><polyline points="6 9 12 15 18 9" /></svg>
}

function IconLink({ href, label, children, badge }: { href: string; label: string; children: React.ReactNode; badge?: number }) {
  return (
    <Link href={href} aria-label={label} title={label} className="relative inline-flex items-center justify-center w-9 h-9 rounded-full text-stone-700 hover:text-[#FF5722] hover:bg-stone-100 transition-colors">
      {children}
      {badge && badge > 0 ? (
...
function LanguagePicker() {
  ...
  return (
    <div className="relative" ref={ref} style={{ zIndex: 9999 }}>
      <button type="button" onClick={() => setOpen((v) => !v)} className="inline-flex items-center gap-1 h-10 px-3 rounded-full text-stone-700 hover:text-[#FF5722] hover:bg-stone-100 transition-colors text-xs font-medium">
        <GlobeIcon />
        <span>{currentLocale}</span>
      </button>
      {open && (
        <div className="absolute right-0 mt-1 w-36 bg-white border border-stone-200 rounded-xl shadow-xl py-1" style={{ zIndex: 9999 }}>
          {LANGUAGES.map((lang) => (
            <Link key={lang.code} href={getLocalePath(lang.code)} onClick={() => setOpen(false)} className={`w-full text-left px-3 py-1.5 text-sm hover:bg-stone-50 flex items-center ${lang.code === currentLocale ? 'text-[#FF5722] font-medium' : 'text-stone-700'}`}>
  ...
  return (
    <div className="flex items-center gap-3">
      <form action={`/${locale}`} method="GET" className="relative flex-1">
        <input type="text" name="q" placeholder={locale === 'vi' ? 'Tìm kiếm...' : 'Search...'} className="w-full h-10 pl-10 pr-4 text-sm bg-stone-100 border border-stone-200 rounded-full text-stone-900 placeholder-stone-500 focus:outline-none focus:ring-2 focus:ring-[#FF5722] focus:bg-white transition-colors" />
        <button type="submit" className="absolute left-0 top-0 h-10 w-10 flex items-center justify-center text-stone-500 hover:text-[#FF5722] transition-colors flex-shrink-0" aria-label={locale === 'vi' ? 'Tìm kiếm' : 'Search'}>
          <SearchIcon />
        </button>
      </form>
      <button type="button" onClick={onToggleFilters} className="inline-flex items-center justify-center h-10 w-10 flex-shrink-0 bg-stone-100 border border-stone-200 rounded-full text-stone-700 hover:text-[#FF5722] hover:bg-stone-50 transition-colors" title="Toggle filters">
        <FilterIcon />
      </button>
    </div>
  )
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <ul className="flex items-center overflow-x-auto">
          {STATIC_CATEGORIES.map((cat: Category) => {
            ...
            return (
              <li key={cat.slug} className="relative flex-shrink-0" onMouseEnter={() => handleMouseEnter(cat.slug)} onMouseLeave={handleMouseLeave}>
                <Link href={`/${locale}/category/${cat.slug}`} className={`flex items-center gap-1 px-4 py-3 text-sm font-medium whitespace-nowrap border-b-2 transition-colors duration-150 ${isActive ? 'text-[#FF5722] border-[#FF5722]' : 'text-stone-600 border-transparent hover:text-[#FF5722]'}`}>
                  <span className="mr-1 text-base leading-none">{CATEGORY_ICONS[cat.slug]}</span>
                  {label}
                  {hasSub && <ChevronDown active={isActive} />}
                </Link>
                {hasSub && isActive && (
                  <div className="absolute top-full left-0 bg-white border border-stone-200 rounded-2xl shadow-2xl overflow-hidden" style={{ minWidth: 240, zIndex: 200 }} onMouseEnter={() => handleMouseEnter(cat.slug)} onMouseLeave={handleMouseLeave}>
                    ...
                    <ul className="py-2">
                      {cat.children.map((sub: SubCategory) => {
                        ...
                        return (
                          <li key={sub.slug}>
                            <Link href={`/${locale}/category/${sub.slug}`} className="flex items-center gap-3 px-5 py-2.5 text-sm text-stone-600 hover:text-[#FF5722] hover:bg-orange-50 transition-colors group">
                    ...
                    <div className="px-5 py-2.5 border-t border-stone-100 bg-stone-50">
                      <Link href={`/${locale}/category/${cat.slug}`} className="text-xs font-medium text-[#FF5722] hover:underline">
...
function SiteHeaderInner() {
  ...
  const setupRealtimeChannel = useCallback((userId: string) => {
    ...
    subscriptionRef.current = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'chat_messages' }, (payload: any) => {
        if (payload.new?.receiver_id === userId && !payload.new.is_read) setUnreadCount((prev) => prev + 1)
      })
  ...
  useEffect(() => {
    ...
    const loadUserData = async (userId: string) => {
      try {
        const [msgResult, notifResult] = await Promise.all([
          supabase.from('chat_messages').select('*, conversation:conversations!chat_messages_conversation_id_fkey(status)', { count: 'exact', head: true }).eq('receiver_id', userId).eq('is_read', false).neq('conversation.status', 'completed'),
          supabase.from('notifications').select('*', { count: 'exact' }).eq('user_id', userId).eq('is_read', false).order('created_at', { ascending: false }).limit(10),
        ])
        if (!isMounted) return
        ...
        else { setNotifCount(notifResult.count ?? 0); setNotifs((notifResult.data as NotifRow[]) ?? []) }
      } catch (err) { console.error('Unexpected error loading user data:', err) }
    }
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => {
      ...
      setUser(currentUser)
      if (currentUser) { loadUserData(currentUser.id); setupRealtimeChannel(currentUser.id) }
      else {
        setUnreadCount(0); setNotifCount(0); setNotifs([])
    ...
    supabase.auth.getUser().then(({ data: { user: cu } }) => {
      ...
      setUser(cu); loadUserData(cu.id); setupRealtimeChannel(cu.id)
    })
    ...
    <header className="bg-white shadow-md border-b border-stone-100 sticky top-0" style={{ zIndex: 9000 }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
        <div className="flex items-center justify-between gap-3">
          <Link href={`/${locale}`} className="text-2xl font-black tracking-tight select-none whitespace-nowrap flex-shrink-0">
          ...
          <div className="flex items-center gap-1 flex-shrink-0">
            {user ? (
              <>
                <Link href={`/${locale}/listings/new`} className="inline-flex items-center gap-1 bg-[#FF5722] text-white text-sm font-medium px-4 py-2.5 rounded-full hover:bg-[#E64A19] transition-colors shadow-sm">
                ...
                <div className="relative" ref={notifRef}>
                  <button onClick={() => setShowNotifs((v) => !v)} className="relative inline-flex items-center justify-center w-9 h-9 rounded-full text-stone-700 hover:text-[#FF5722] hover:bg-stone-100 transition-colors">
                    <BellIcon />
                    {notifCount > 0 && (
                      <span className="absolute top-0 right-0 flex items-center justify-center w-4 h-4 text-[9px] font-bold text-white bg-[#FF5722] rounded-full border border-white">
                      ...
                      <div className="px-4 py-3 border-b border-stone-100 flex items-center justify-between">
                        ...
                        {notifCount > 0 && (
                          <button onClick={async () => {
                            ...
                            setNotifCount(0); setNotifs([]); setShowNotifs(false)
                          }} className="text-xs text-[#FF5722] hover:underline">Mark all read</button>
                        )}
                      </div>
                      {notifs.length === 0 ? (
                        ...
                        <div className="max-h-80 overflow-y-auto">
                          {notifs.map((n) => (
                            <a key={n.id} href={n.link || '#'} onClick={async () => {
                ...
                <IconLink href={`/${locale}/favorites`} label={tNav('wishlist')}><HeartIcon /></IconLink>
                <IconLink href={`/${locale}/orders`} label={tNav('orders')}><BagIcon /></IconLink>
                <IconLink href={`/${locale}/profile`} label={tNav('account')}><UserIcon /></IconLink>

                <LanguagePicker />

                <form action={`/${locale}/auth/signout`} method="post">
                  <button type="submit" className="hidden sm:block text-xs text-stone-500 hover:text-stone-900 px-2 py-1.5 ml-1">
                    {tNav('signout')}
                  </button>
                </form>
              </>
              ...
              <>
                <LanguagePicker />
                <Link href={`/${locale}/auth/login`} className="text-sm text-stone-700 hover:text-stone-900 px-2">{tNav('signin')}</Link>
                <Link href={`/${locale}/auth/signup`} className="bg-[#FF5722] text-white text-sm px-4 py-2.5 rounded-full hover:bg-[#E64A19] transition-colors shadow-sm">{tNav('signup')}</Link>
              </>
            )}
          </div>
        </div>
