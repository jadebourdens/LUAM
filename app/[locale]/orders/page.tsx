      ...
      {confirmPaymentOrder && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl p-6 max-w-sm mx-4 text-center shadow-xl">
            ...
            <p className="text-gray-600 mb-2">Confirm you received payment for:</p>
            <p className="font-bold text-[#FF5722] mb-1">{confirmPaymentOrder.listing?.title}</p>
      ...
      <div className="min-h-screen bg-gray-50 p-6">
        ...
        <div className="space-y-3">
          {data.map((o) => {
            ...
            return (
              <div key={o.id} className={`bg-white rounded-xl shadow-sm border p-4 ${o.status === 'pending' && tab === 'selling' ? 'border-l-4 border-l-[#FF5722] border-orange-100' : 'border-gray-100'}`}>
                ...
                {tab === 'selling' && o.status === 'pending' && (
                  <button onClick={() => setConfirmPaymentOrder(o)} className="mt-3 bg-emerald-600 text-white px-3 py-2 rounded-lg text-sm">{t('confirm_bank')}</button>
                )}
                
                {tab === 'selling' && o.status === 'paid' && (
                  <div className="mt-3 flex gap-2">
                    <input placeholder="Tracking #" value={trackingInputs[o.id] || ''} onChange={(e) => setTrackingInputs(prev => ({ ...prev, [o.id]: e.target.value }))} className="border rounded-lg px-3 py-2 text-sm" />
                    <button onClick={() => updateStatus(o.id, 'shipped', trackingInputs[o.id])} className="bg-[#FF5722] text-white px-3 py-2 rounded-lg text-sm">{t('mark_shipped')}</button>
                  </div>
                )}
              </div>
            )
